...
export default class ProductListing {
  ...
  prepareTemplate(template, product) {
    ...
    if (product.SuggestedRetailPrice > product.ListPrice) {
      let percentOff = Math.round(100 * (product.SuggestedRetailPrice - product.ListPrice) / product.SuggestedRetailPrice)
      template.querySelector(".product-card__discount").innerHTML = `${percentOff}% Off`
    }
